...
task = 'Hab'
runs = np.array([1,2,3,4,5,6])
condnums = np.array([1,2,3,4,5,6,7])
condlabel = ('CSpS', 'CSpNoS', 'CSpI', 'CSm', 'CSmI', 'shock', 'CSmEx')
...
for sub in subs:
	...
	for run in runs:
		...
		for i in np.arange(0,len(condnums)):
			...
			if (condlabel[i] != 'CSmEx' and condlabel[i] != 'CSmI' and condlabel[i] != 'shock'):
			...
			elif condlabel[i] == 'shock':
				df_cond = df[df.ShockOn==1]
				numtrials = len(df[df.ShockOn==1])		
				## Create array in 3 column format for condition
				trialon = np.zeros([numtrials,3])
				trialon[:,0] = df_cond.GaborOn+3.995
				trialon[:,1] = 0.05
				trialon[:,2] = paramweight
			elif condlabel[i] == 'CSmI':
				targettrials = df.Condition==condnums[i]
				firsttrial = df['Trial'] != 1
				lasttrial = df['Trial'] != df['Trial'].values[-1]
				triallist=targettrials&firsttrial&lasttrial
				df_cond = df[triallist]
				numtrials = len(df_cond)
				## Create array in 3 column format for condition
				trialon = np.zeros([numtrials,3])
				trialon[:,0] = df_cond.GaborOn
				trialon[:,1] = eventdur
				trialon[:,2] = paramweight
			elif condlabel[i] == 'CSmEx':
				firsttrial = df['Trial'] == 1
				lasttrial = df['Trial'] == df['Trial'].values[-1]
				df_cond = df[firsttrial | lasttrial]
				numtrials = len(df_cond)
				## Create array in 3 column format for condition
				trialon = np.zeros([numtrials,3])
				trialon[:,0] = df_cond.GaborOn
